...
return this}}(jQuery);}
qc.Model=Backbone.NestedModel.extend({defaults:'',initialize:function(){},isJson:function(str){try{JSON.parse(str);}catch(e){return false;}
return true;},updateForm:function(data){preloaderStop();if(parseInt(config.general.debug)){}
...
var json=this.toJSON();var that=this;$.post('index.php?route=extension/d_quickcheckout/payment_address/update',{'payment_address':json.payment_address},function(data){that.updateForm(data);},'json').error();},updateField:function(name,value){clearTimeout(qc.payment_address_waiting);this.set(name,value);var that=this;var json=this.toJSON();qc.payment_address_waiting=setTimeout(function(){$.post('index.php?route=extension/d_quickcheckout/payment_address/update',{'payment_address':json.payment_address},function(data){that.updateForm(data);},'json').error();},500);},validate:function(attrs,options){return false;},handleError:function(model,error){var that=this;_.each(error,function(element){that.model.set(element.field+"."+element.key,element.value);});}});qc.PaymentAddressView=qc.View.extend({initialize:function(e){this.template=e.template;qc.event.bind("update",this.update,this);this.render();},events:{'change input[type=radio].payment-address':'changeAddress','change select.country_id':'changeCountry','change select.zone_id':'zoneDelay'},template:'',fields:'',changeAccount:function(account){if(this.model.get('account')!==account){this.model.changeAccount(account);this.render();}},changeAddress:function(e){this.model.changeAddress(e.currentTarget.value);this.render();if(parseInt(config.general.analytics_event)){ga('send','event',config.name,'update','payment_address.changeAddress');}
...
preloaderStart();},render:function(){this.focusedElementId=$(':focus').attr('id');$(this.el).html(this.template({'model':this.model.toJSON()}));$('#'+this.focusedElementId).focus();},update:function(data){if(data.shipping_method){this.model.set('shipping_method',data.shipping_method);}
...
if(parseInt(config.general.analytics_event)){ga('send','event',config.name,'click','confirm.confirm');}},paymentConfirm:function(){preloaderStart();$(document).ajaxStop(function(){preloaderStop();});},changeAccount:function(account){if(this.model.get('account')!==account){this.model.changeAccount(account);this.render();}},update:function(data){var that=this;if(data.confirm){this.model.set('confirm',data.confirm);}
if(typeof(data.show_confirm)!=='undefined'){this.model.set('show_confirm',data.show_confirm);}
